...
function ClientState({ children }) {
    ...
    //get Sidebar data
    const sidebar = async () => {
        const res = await Axios.get("https://learn-in-com.herokuapp.com/client/title");
    ...
    const courseData = async (collection) => {
        const res = await Axios.get("https://learn-in-com.herokuapp.com/client/courseData", { params: { collection: collection } });
        if(res.data.result === "Ok"){
            dispatch({
    ...
    const titleWiseData = async(collection,id)=>{
        const res = await Axios.get("https://learn-in-com.herokuapp.com/client/getDataUsingTitle",{params:{collection:collection,id:id}})
       if(res.data.result === "Ok"){
           dispatch({
               type:TITLE_DATA,
               payload:res.data
